...
const LoginButton: React.FC<LoginButtonProps> = ({ variant, id }) => {
  ...
  const baseClasses = 'flex items-center gap-2 font-semibold rounded justify-center min-w-[180px]';
  const headerClasses = 'px-4 py-2 bg-green-100 text-green-700 hover:bg-green-200';
  const defaultClasses = 'py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white';
  ...
  return (
    <button
      ...
      onClick={handleLogin}
      className={
        baseClasses + ' ' + (variant === 'header' ? headerClasses : defaultClasses)
      }
      disabled={loading}
    >
      {loading ? (
